...
### 5.5 召回率与精确率：检索质量怎么度量

「召回」这个词有两个层次，面试常混在一起考，先分清楚：

1. **动作层**：召回（Retrieval）就是「把相关片段从知识库里捞回来」这个检索过程，RAG 里的「召回阶段」指的就是这一步；
2. **指标层**：召回率（Recall）是衡量「捞得全不全」的指标，和精确率（Precision）是一对，源自信息检索，RAG 直接沿用。

**定义 + 例子：**

```
假设知识库里和当前问题真正相关的片段有 10 个，
检索器返回了 top-8，其中 6 个真相关、2 个无关：

召回率   Recall   = 6 / 10 = 60%  → 该找的 10 个只找回了 6 个（漏了没）
精确率   Precision = 6 / 8  = 75%  → 返回的 8 个里 6 个真相关（偏没偏）
```

| 指标 | 公式 | 追问点 | 一句话 |
|---|---|---|---|
| 召回率 Recall | 检索到的相关片段 / 所有相关片段 | 漏了没 | 该找的都找到了吗 |
| 精确率 Precision | 检索到的相关片段 / 检索到的所有片段 | 偏了没 | 找出来的都对吗 |

**RAG 里为什么召回率比精确率更关键：**

- 召回率低 → 关键片段漏掉 → LLM 上下文里没有答案 → 幻觉 / 答不全 / 答错，这是硬伤；
- 精确率低 → 多带了几条噪声片段，顶多浪费 token、靠 LLM 自己过滤，问题小得多；
- 所以工程上一贯是「**先多召回保不漏，再 Rerank 精排保准**」：第一阶段放宽 top-k（甚至多路召回），第二阶段用 Cross-Encoder 这类强模型重排，取前 5~12 条喂给 LLM。

**检索评估指标速查：**

- **Recall@K**：前 K 个结果里，找回了多少比例的相关片段。注意分母是「所有相关片段总数」，不是 K——拿 K 当分母算出来的是 Precision，别写反（rag-eval-bench 明确提醒过这一点）。
- **Precision@K**：前 K 个结果里真正相关的占比。
- **Hit Rate**：前 K 个结果里「至少命中 1 个相关片段」的查询占比。
- **MRR**：第一个相关片段排在第几位的倒数求平均，看「排得好不好」。
- **NDCG@K**：考虑排序位置权重的相关度得分，位置越靠前权重越高。

**召回不理想怎么排查（美团 2026-08 Agent 一面原题）：**

按链路从上往下查：切分是否把语义切断 → Embedding 是否与语料匹配 → 是否只靠单一向量检索、漏了精确词（型号/缩写） → top-k 是否太小 → 相似度阈值/度量是否配错 → 是否缺 query 改写、多路召回、Rerank。

**提升召回率的常见手段：**

1. **多路召回 + RRF 融合**：向量（语义）+ BM25（关键词）两路跑，RRF 按排名倒数和合并，覆盖面明显更宽（京东面经里「混合检索把召回率提升 17%」是高频案例）。
2. **Query 改写 / 扩展**：把用户口语问题改写成多个检索变体，或用 HyDE 先让 LLM 生成假设答案再拿去检索。
3. **Rerank 精排**：召回放宽、重排收紧，Cross-Encoder 对召回候选重新打分。
4. **调整切分与 overlap**：结构边界切 + 10%~20% overlap，避免关键信息被切断。

**面试话术：** "召回是 RAG 的第一道关口，召回率衡量该找的片段找全没有，精确率衡量找出来的准不准。RAG 场景我倾向召回优先、精确靠 Rerank 兜底——多路召回先保不漏，重排再保准，最后用 Recall@K 做发布门禁防止召回质量回退。"

**参考：** 美团 Agent 一面「RAG 召回不理想怎么排查」（2026-08）— https://m.nowcoder.com/discuss/919673484509184000 ；京东「混合检索召回率提升 17%」面经复盘（2026-03）— https://gitcode.csdn.net/69c4d38c54b52172bc64aedc.html ；Recall@K / MRR / NDCG 三指标详解（腾讯云 2026-04）— https://cloud.tencent.cn/developer/article/2658116 ；「召回测试才是 RAG 质量底座」（阿里云 2026-06）— https://developer.aliyun.com/article/1741398
...
## 九、Agent 评测（Evaluation）

### 9.1 为什么 Agent 比传统软件难测？

- **输出非确定**：同样输入，LLM 可能给出不同答案，不能用固定断言「等于某个值」；
- **是多步轨迹**：好坏不仅看最终答案，还看中间每一步（调了哪个工具、参数对不对、顺序对不对）；
- **依赖外部环境**：工具、知识库、API 状态都会影响结果，可复现性差；
- **好坏是语义层面**：答案「对/错」往往是模糊的，需要语义判断而非布尔比较。

结论：Agent 评测不能只测「最终输出」，要**分层次、分阶段**测。

### 9.2 评测的三个层次（对应 Agent 链路）

| 层次 | 测什么 | 例子 |
|---|---|---|
| 组件/单元层 | 单个能力点 | 意图识别准不准、工具选没选对、RAG 召回、输出 JSON 格式 |
| 轨迹层 | 整个多步执行过程 | 工具调用顺序、是否走了正确路径、Loop 是否收敛、有没有多余步骤 |
| 任务/端到端层 | 最终结果 | 答案对不对、任务完没完成、有没有幻觉 |

面试时先抛这个框架，再往下展开，比只背指标有层次感。

### 9.3 评测方法

1. **黄金数据集（Golden Dataset）+ 规则/断言**：手建典型用例集，每条含「输入 + 上下文 + 期望行为」，对可确定的部分（工具名、参数、JSON 结构）做确定性校验。这是字节一面标准答案的第一环。
2. **LLM-as-a-Judge**：用更强的 LLM 当评委，按 rubric 打分。要警惕三类 bias——**verbosity bias**（偏爱更长的答案/思考链）、**position bias**（偏爱排在前面/后面的候选）、**self-bias**（偏爱自己模型风格）。缓解手段：明确 rubric + few-shot 示例 + 随机化顺序 + 多次取平均。
3. **Agent-as-Judge**：2026 趋势，judge 本身也做成 agent（可调用工具去验证答案、查证据），比单纯 LLM 打分更可靠，但仍需工程化约束。
4. **开源 Benchmark**：AgentBench（多步交互 + 工具）、SWE-bench（修真实 GitHub issue）、GAIA（现实世界问答）、WebArena（网页任务）、τ-bench / Terminal-Bench（工具/终端操作）。适合评估通用能力基线，但**不能替代自家业务评测集**。
5. **人工评测**：金标准但贵，用于抽样、最终确认和校准 LLM judge 的阈值。

### 9.4 离线 vs 在线评测（Decagon 两阶段范式）

**离线（Offline）——开发期回归：**

- 用评测集跑全链路，看 Recall@K、judge score、工具调用准确率、关键任务通过率；
- 价值：改切片策略、换 Embedding、换 Prompt、升级模型前先跑一遍，防「召回质量悄悄回退」；
- 落地：**evals as code**，评测写成 pytest 风格的用例接入 CI/CD，作为发布门禁。

**在线（Online）——生产期验证：**

- A/B 测试分流量，对比新旧版本；
- 看业务指标：解决率、任务完成率、CSAT（满意度）、转人工率、p50/p95 延迟、每任务 token 成本；
- 生产流量抽样评测：线上真实请求抽样回放给 LLM judge 打分，补离线集覆盖不到的分布。

一句话：**离线管「能不能上」，在线管「上了好不好」**，两者串成迭代闭环。

### 9.5 指标怎么分

- **最终答案**：正确性、Faithfulness（忠实于检索内容、不幻觉）、相关性、完整性；
- **轨迹**：工具调用准确率、步骤数、路径正确性、Loop 是否收敛；
- **RAG**：Recall@K、Precision@K、引用准确率（答案有没有证据支撑）；
- **成本/效率**：延迟（p50/p95）、token 消耗、每任务成本、解决率、转人工率。

### 9.6 工程落地与工具选型

- **评测集四类**：快乐路径、边界 case、对抗样本、线上翻车 case 回放——最后一类最值钱，生产每出一次 bug 就回灌进评测集。
- **失败分析闭环**：生产翻车 → 定位是召回漏了 / 工具错了 / Prompt 弱了 → 补用例 → 回归，形成「评测集越跑越厚」的正循环。
- **工具**：DeepEval（pytest 风格，CI 友好）、RAGAS（RAG 评测专精）、OpenAI Evals、Promptfoo、Braintrust；可观测平台 LangSmith / Langfuse（顺带做在线采样与打分）。
- **选型口诀**：全链路都在 LangChain/LangGraph → LangSmith；CI 是记录系统、要 pytest 体验 → DeepEval；重点在 RAG 检索质量 → RAGAS；不想自建 judge → OpenAI Evals / Promptfoo。

### 9.7 面试速答

**字节一面「怎么对 Agent 做评估」**：三层评测（组件/轨迹/端到端）+ 黄金数据集 + LLM-as-a-Judge（带 rubric、防 bias）+ 开源 benchmark 做通用基线，最后强调自家业务必须建私有评测集。

**美团「上线前怎么测、上线后怎么知道好不好」**：上线前离线三层回归 + Recall@K/judge score 做发布门禁；上线后 A/B 测试看解决率/CSAT/延迟/成本，生产流量抽样评测持续校准。

**别踩的坑（字节连环崩复盘）**：没做 baseline 评测、异常路径没设计、评测集只用 happy path——这三点在字节 Agent 面试里被连续追问，务必主动讲 baseline + 回归门禁 + 对抗用例。

**参考：** 字节一面「怎么对 Agent 做评估」复盘（2026-08）— http://www.daiyunguke.com/news/102168 ；美团 Agent 一面工程落地（2026-08）— https://leetcode.cn/discuss/post/4014695/ ；智能体评测体系「从答对没到过程对不对」（CSDN 2026-08）— https://blog.csdn.net/thesky123456/article/details/163758447 ；AI Agent Evaluation 三层次（Morph 2026-06）— https://www.morphllm.com/ai-agent-evaluation ；Decagon 评测引擎两阶段（2025-07）— https://decagon.ai/blog/evaluation-engine-ai-agents ；Agent 评测框架选型（FutureAGI 2025-10）— https://futureagi.com/blog/agent-evaluation-frameworks-2026/
...
| Agent 评测怎么做？ | 三层评测（组件/轨迹/端到端）+ 黄金数据集 + LLM-as-a-Judge + 离线回归门禁 + 在线 A/B |
| 你项目怎么用的？ | 见第十节话术 |
