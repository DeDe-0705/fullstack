...
import type { AgentMessage, AgentToolTrace, AgentUsage, Paginated } from '../lib/agent'
...
export function AgentChat() {
  ...
  }, [messagesQuery.data, streaming, streamContent, streamReasoning, streamTools])
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (conversation) => {
      ...
      setLastToolCalls([])
      setLastMeta(null)
      queryClient.invalidateQueries({ queryKey: ['agent', 'conversations', userId] })
    },
  })
  ...
  async function runChat(message: string) {
    ...
    const appendMessage = (id: string) => {
      queryClient.setQueryData<Paginated<AgentMessage>>(
        ...
        (old) =>
          old
            ? { ...old, items: [...old.items, optimisticMessage], total: old.total + 1 }
    ...
    <Layout style={{ height: 'calc(100vh - 96px)', background: '#fff' }}>
      ...
      >
        <Flex vertical style={{ height: '100%' }}>
          <Flex justify="space-between" align="center" style={{ padding: '12px 16px' }}>
            ...
            <Button type="text" size="small" icon={<LogoutOutlined />} onClick={clearSession}>
              退出
            </Button>
          </Flex>
              ...
              renderItem={(conversation) => (
                <List.Item
                  ...
                  style={{
                    ...
                    padding: '12px 16px',
                    background:
                      conversation.id === conversationId ? '#e6f4ff' : undefined,
      ...
      <Content style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <Flex vertical gap={12} style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
          ...
          {(streaming || streamReasoning || streamContent || streamTools.length > 0) && (
            <Flex justify="flex-start">
              <Card size="small" style={{ maxWidth: '75%', background: '#fafafa' }}>
                <Flex vertical gap={8}>
                  ...
                  {!streaming &&
                    (streamReasoning || streamContent || streamTools.length > 0) && (
                      <Typography.Text type="secondary" style={{ fontSize: 12 }}>
                        已停止生成
                      </Typography.Text>
                    )}
        ...
        <Flex vertical gap={8} style={{ padding: 12, borderTop: '1px solid #f0f0f0' }}>
