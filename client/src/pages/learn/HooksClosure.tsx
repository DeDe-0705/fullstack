    ...
    <Flex vertical gap={24}>
      ...
      <Card title="演示 1：闭包陷阱——定时器读到旧值" size="small">
        <Space direction="vertical" style={{ width: '100%' }}>
          <Typography.Paragraph type="secondary">
            点击「+1」两次，然后在 3 秒内再点一次「+1」。观察定时器打印的 count 和页面显示的 count 不一致。
          </Typography.Paragraph>
          <Space>
      ...
      <Card title="演示 2：函数式更新——prev 永远是最新值" size="small">
        <Space direction="vertical" style={{ width: '100%' }}>
          <Typography.Paragraph type="secondary">
            对比演示 1：同样点击后 3 秒，但用 setCount(prev =&gt; prev + 1)，拿到的是最新值。
          </Typography.Paragraph>
      ...
      <Card title="演示 3：setState 批量更新——同一事件里多次 setState" size="small">
        <Space direction="vertical" style={{ width: '100%' }}>
          <Typography.Paragraph type="secondary">
            React 18+ 自动批处理：同一事件处理器里的多次 setCount(count + 1) 只会让 count 加 1（基于同一旧值）。
            而函数式更新会依次基于最新 prev 计算。
          </Typography.Paragraph>
          <Space>
            <Button onClick={handleBatching}>连续 3 次 setCount(count+1)</Button>
      ...
      <Card title="演示 4：同步还是异步？——事件处理器末尾直接读 count" size="small">
        <Space direction="vertical" style={{ width: '100%' }}>
          <Typography.Paragraph type="secondary">
            点击后观察日志：函数式更新能依次拿到最新 prev，但事件处理器末尾直接读 count 仍然是旧值。
          </Typography.Paragraph>
      ...
      >
        <Flex vertical gap={4}>
          {logs.length === 0 && (
            <Typography.Text type="secondary">暂无日志，点击上方按钮开始演示</Typography.Text>
          )}
          {logs.map((log, i) => (
            <Typography.Text key={i} style={{ fontSize: 12, fontFamily: 'monospace' }}>
              {log}
            </Typography.Text>
          ))}
        </Flex>
