...
export function UseEffectLifecycle() {
  ...
  return (
    <Flex vertical gap={24}>
      <div>
        <Typography.Title level={3}>useEffect 生命周期与依赖数组</Typography.Title>
        <Alert
          type="info"
          showIcon
      ...
      <Card title="操作区" size="small">
        <Space direction="vertical" style={{ width: '100%' }}>
          <Space>
            ...
            <Button onClick={() => setText(t => t + 'a')}>text 追加字符</Button>
            <Button onClick={clear}>清空日志</Button>
          </Space>
          <Typography.Paragraph type="secondary">
            观察日志中：
            <br />1. count 变化时，useEffect([count]) 先执行清理函数（打印旧 count），再执行新 effect
            <br />2. text 变化时，useEffect([count]) 不会重新执行
            <br />3. useLayoutEffect 总是先于 useEffect 打印
            <br />4. StrictMode 下挂载 effect 执行两次（开发模式特有）
